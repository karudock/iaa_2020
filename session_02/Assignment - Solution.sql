...
SELECT
    *,
    ST_Distance(ST_GeogPoint(end_longitude, end_latitude), ST_GeogPoint(start_longitude, start_latitude)) AS dist_meters
FROM
    (
    SELECT
        c.*,
    ...
    LEFT JOIN
        `bigquery-public-data.austin_bikeshare.bikeshare_stations` d ON cast(c.end_station_id as STRING) = cast(d.station_id as STRING)
    ) e
